...
export default function DashboardPage({ isAdmin = false }) {
  const { token, userEmail, userRole, logout } = useAuth();
  ...
  useEffect(() => {
    if (showDeleted && token && isAdminUser) {
      fetchDeletedEmployees();
    }
  }, [showDeleted, token, isAdminUser]);
    ...
    <div className="min-h-screen bg-[#F5F5F0] font-sans">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            <span className="text-sm text-[#5A5A40]/60 hidden md:block">
              Logged in as <span className="font-medium text-[#1A1A1A]">{userEmail}</span> • <span className="font-medium text-[#1A1A1A]">{isAdminUser ? 'Admin' : 'User'}</span>
            </span>
            <button
      ...
      <main className="max-w-7xl mx-auto px-6 pt-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h2 className="text-4xl font-bold text-[#1A1A1A] mb-2 font-serif">
              {isAdminUser ? 'Admin Employee Panel' : 'Employee List'}
            </h2>
          </div>
